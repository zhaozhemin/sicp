...
;;       (and (job ?person (?division . ?sub-divsion))
;;            (meeting ?division ?day-and-time)))

;; (meeting-time (Hacker Alyssa P) (Wednesday ?y))

; TODO ex 4.60

; Because it scans all assertions.
...
; No, because (last-pair ?x (3)) has a infinite number of answers.

; TODO ex 4.63

; ex 4.64

;; The ?middle-manager and ?boss in the recursive call are both
;; unbound, thus an answer cannot be derived. So it's essentially a
;; recursive call with no base case, thus an infinite loop.

; ex 4.65

; One for Eben, three for Ben, because Ben supervises three people.

; ex 4.66

; Some queries may give repetitive results. Filter the stream to
; remove frames which have the same value for certain variables.
...
;;                (append-to-form (reverse ?y ?g) ?x ?z)))

; TODO ex 4.69

; ex 4.70

; THE-ASSERTIONS will be a stream containing only the newly added
...
(define (simple-query query-pattern frame-stream)
  (stream-flatmap
   (lambda (frame) (stream-append-delayed
...
(define (disjoin-not-delayed disjuncts frame-stream)
  (if (empty-disjunction? disjuncts)
      the-empty-stream
      (interleave
       (qeval (first-disjunct disjuncts) frame-stream)
...
(put 'or 'qeval disjoin-not-delayed)
...
(define (flatten-stream stream)
  (if (stream-null? stream)
      the-empty-stream
      (interleave-delayed
       (stream-car stream) (delay (flatten-stream (stream-cdr stream))))))
